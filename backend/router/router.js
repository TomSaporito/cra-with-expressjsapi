...
module.exports = function(app, express){
    ...
    //change route to resources dynamically
    const staticRoute = process.env.NODE_ENV === 'development'? '/../../client/public/' : '/../../client/build/';
    
    console.log(staticRoute);
    app.set('views', path.join(__dirname + staticRoute));//directory to find views
    app.set('view engine', 'jade');//template engine is jade
    app.use(express.static(path.join(__dirname + staticRoute)));//get static files form public directory
